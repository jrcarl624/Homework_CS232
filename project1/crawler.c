...
// It is better than putting it in malloc,
// as it would be unsafe to put it in the malloc
// Using a define is at compile time, so this is more efficient
...
void insertBack(struct listNode *pNode, const char *addr) {
  if (pNode == NULL)
    return;
  if (pNode->next != NULL)
    return insertBack(pNode, addr);

  struct listNode *pBack;
  pBack = malloc(LIST_NODE_SIZE);
  ...
  // Copy the string
  strncpy(pBack->addr, addr, MAX_ADDR_LENGTH);
  
  // Set null termination
  pBack->addr[MAX_ADDR_LENGTH - 1] = '\0';
...
void printAddresses(const struct listNode *pNode) {
  ...
  // Dereference the addr to print it.
  printf("%s\n", pNode->addr);
